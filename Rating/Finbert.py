...
API_token = API.get_token("Finbert")
...
logging.basicConfig(filename='errors_fin.log', level=logging.ERROR)
start_time = time()
average_time = [0]
max_text_len = 2000
...
with sqlite3.connect(db_path) as conn:
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS news_rating_finbert
    ...
    cursor.execute('''SELECT nc.data_id, nc.title, nc.text
        FROM news_content AS nc LEFT JOIN news_rating_finbert ON nc.data_id = news_rating_finbert.data_id
        WHERE news_rating_finbert.title_negative IS NULL OR news_rating_finbert.text_negative IS NULL;''')
    for (data_id, title, text) in cursor.fetchall():
        try:
            query_list = [title]
            query_list.extend(split_text(text))
            ...
            cursor.execute('''INSERT OR REPLACE INTO news_rating_finbert
            ...
            if "currently loading" in response['error']:
                ...
                API_token = API.update_token("Finbert")
                sleep(5)
